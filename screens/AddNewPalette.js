...
const AddNewPalette = ({ navigation }) => {
  ...
  const handleUpdate = useCallback(
    (newValue, color) => {
      if (newValue === true) {
        return setSelectedColors((current) => [...current, color]);
      } else {
        setSelectedColors((current) =>
          current.filter((selected) => selected.colorName !== color.colorName),
        );
      }
    },
  ...
  const handleSubmit = useCallback(() => {
    if (!value) {
      ...
      navigation.navigate('Home', {
        newPallete: { paletteName: value, colors: selectedColors },
      });
    }
